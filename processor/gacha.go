...
// 抽取队长
func (p *Processor) GachaCaptain(pc *ProcessContext, num int) ([]string, error) {
	var captains []model.AbyssCaptain
	// 添加 family_id 查询条件
	if err := p.DB.Where(&model.AbyssCaptain{FamilyID: pc.Family.ID, Enabled: true}).Find(&captains).Error; err != nil {
...
// 抽取成员
func (p *Processor) GachaMember(pc *ProcessContext, num int, filterParams []string) ([]string, error) {
	filter := ParseFilter(filterParams)
	stat := p.DB.Where("family_id = ?", pc.Family.ID).Order("damage DESC") // 添加 family_id 查询条件
